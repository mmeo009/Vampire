        ...
        foreach (string asset in importedAssets)
        {
            ...
            using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                foreach (string sheetName in sheetNames)
                {
					...
					cell = row.GetCell(5); p.taskAmount = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(6); p.taskCondition = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(7); p.taskReturnAmount = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(8); p.onlyFor = (cell == null ? "" : cell.StringCellValue);
